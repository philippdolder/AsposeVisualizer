// --------------------------------------------------------------------------------------------------------------------
// <copyright file="FooterProxyFacts.cs" company="Philipp Dolder">
//   Copyright (c) 2014
//
...
namespace AsposeVisualizer
{
    ...
    public class FooterProxyFacts
    {
        private const string FooterType = "Type";
        private readonly FooterProxy testee;

        public FooterProxyFacts()
        {
            this.testee = new FooterProxy(FooterType);
        }

        [Fact]
        public void HasType()
        {
            this.testee.Type.Should().Be(FooterType);
        }
        ...
        [Fact]
        public void VisitsChildrenBetweenFooterStartAndEnd()
        {
            ...
            using (var scope = Fake.CreateScope())
            {
                ...
                {
                    A.CallTo(() => visitor.VisitFooterStart(this.testee)).MustHaveHappened();
                    A.CallTo(() => child.Accept(visitor)).MustHaveHappened();
                    A.CallTo(() => visitor.VisitFooterEnd(this.testee)).MustHaveHappened();
                }
            }
        }
