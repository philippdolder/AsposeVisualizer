// --------------------------------------------------------------------------------------------------------------------
// <copyright file="GroupShapeProxyFacts.cs" company="Philipp Dolder">
//   Copyright (c) 2014
//
...
namespace AsposeVisualizer
{
    ...
    public class GroupShapeProxyFacts
    {
        private const string ShapeName = "name";

        private readonly GroupShapeProxy testee;
        ...
        public void HasName()
        {
            this.testee.Name.Should().Be(ShapeName);
        }
        ...
        [Fact]
        public void VisitsChildrenBetweenGroupShapeStartAndEnd()
        {
            ...
            using (var scope = Fake.CreateScope())
            {
                ...
                using (scope.OrderedAssertions())
                {
                    A.CallTo(() => visitor.VisitGroupShapeStart(this.testee)).MustHaveHappened();
                    A.CallTo(() => child.Accept(visitor)).MustHaveHappened();
                    A.CallTo(() => visitor.VisitGroupShapeEnd(this.testee)).MustHaveHappened();
                }
            }
        }
