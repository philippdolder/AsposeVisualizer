// --------------------------------------------------------------------------------------------------------------------
// <copyright file="ShapeProxyFacts.cs" company="Philipp Dolder">
//   Copyright (c) 2014
//
...
namespace AsposeVisualizer
{
    ...
    public class ShapeProxyFacts
    {
        private const string ShapeName = "name";

        private readonly ShapeProxy testee;
        ...
        public void VisitsChildrenBetweenShapeStartAndEnd()
        {
            ...
            using (var scope = Fake.CreateScope())
            {
                ...
                using (scope.OrderedAssertions())
                {
                    A.CallTo(() => visitor.VisitShapeStart(this.testee)).MustHaveHappened();
                    A.CallTo(() => child.Accept(visitor)).MustHaveHappened();
                    A.CallTo(() => visitor.VisitShapeEnd(this.testee)).MustHaveHappened();
                }
            }
        }
